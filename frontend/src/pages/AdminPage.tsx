...
function Field({
  ...
  return (
    <label className="grid gap-1.5">
      ...
      {children}
      {help && !error && (
        <span className="text-xs leading-5 text-ink/50">{help}</span>
      )}
      {error && (
...
function prepareContactUrl(value: string, url: string) {
  ...
  if (/^(https?:\/\/|www\.|[a-z0-9-]+\.)/i.test(value.trim()))
...
function ProfileAdmin() {
  ...
  async function uploadInto(
    field: "landing_photo_url" | "resume_url",
    file?: File,
  ) {
    if (!file) return;
    setStatus(null);
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-ink/60">
        Fill this once and the public landing page updates automatically. Image
        and resume fields can use uploads or normal links.
      </p>
      <form onSubmit={submit} className="mt-6 grid gap-4">
        ...
        <Field
          label="Name"
          error={errors.name}
          help="Your display name on the portfolio."
        >
          <Input
        ...
        <Field
          ...
          help="Short sentence shown in the big hero title."
        >
          <Input
        ...
        <Field
          label="Bio"
          error={errors.bio}
          help="A short paragraph about you."
        >
          <Textarea
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            label="Landing photo"
            error={errors.landing_photo_url}
            help="Upload an image or paste a link."
          >
            <div className="flex gap-2">
              <Input
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    landing_photo_url: event.target.value,
                  }))
                }
              />
              <label className="inline-flex h-12 cursor-pointer items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-ink ring-1 ring-ink/10 hover:ring-clay/30">
                ...
                <input
                  ...
                  onChange={(event) =>
                    uploadInto("landing_photo_url", event.target.files?.[0])
                  }
                />
              </label>
            </div>
          ...
          <Field
            ...
            help="Upload a PDF or paste a resume link."
          >
            <div className="flex gap-2">
              <Input
              ...
              <label className="inline-flex h-12 cursor-pointer items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-ink ring-1 ring-ink/10 hover:ring-clay/30">
                ...
                <input
                  ...
                  onChange={(event) =>
                    uploadInto("resume_url", event.target.files?.[0])
                  }
                />
              </label>
            </div>
...
function validateProfile(form: {
  ...
  if (!form.headline.trim())
    errors.headline = "Add a headline for the landing page.";
  if (!form.bio.trim()) errors.bio = "Add a short bio.";
  if (!isValidOptionalUrl(form.landing_photo_url)) {
    errors.landing_photo_url =
      "Use an upload URL, http(s) link, or leave it empty.";
  }
...
function ProjectsAdmin() {
    ...
    try {
      if (editing) {
        await updateProject.mutateAsync({
          id: editing.id,
          payload: payload as Partial<Project>,
        });
        setEditing(null);
  ...
  async function uploadInto(
    field: "cover_image_url" | "gallery_image_urls",
    file?: File,
  ) {
    if (!file) return;
    setStatus(null);
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
      <Card className="p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-ink/60">
          Only title and description are required. The admin will generate the
          slug and summary when you leave them empty.
        </p>
        <form onSubmit={submit} className="mt-6 grid gap-3">
          ...
          <Field
            label="Project title"
            error={errors.title}
            help="Example: FinFlow Dashboard"
          >
            <Input
          ...
          <Field
            label="Short summary"
            help="Optional. If empty, it will be created from the description."
          >
            <Input
          ...
          <Field
            ...
            help="Write the project story, problem, solution, and result."
          >
            <Textarea
            ...
            <div className="flex gap-2">
              ...
              <label className="inline-flex h-12 cursor-pointer items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-ink ring-1 ring-ink/10 hover:ring-clay/30">
                ...
                <input
                  ...
                  accept="image/png,image/jpeg,image/webp,image/gif"
                  onChange={(event) =>
                    uploadInto("cover_image_url", event.target.files?.[0])
                  }
                />
              </label>
            </div>
          ...
          <Field
            ...
            error={errors.gallery_image_urls}
            help="Optional. Upload one by one or paste links separated by commas."
          >
            <div className="flex gap-2">
              <Input
                ...
                onChange={(event) =>
                  setForm((current) => ({
                    ...current,
                    gallery_image_urls: event.target.value,
                  }))
                }
              />
              <label className="inline-flex h-12 cursor-pointer items-center justify-center rounded-xl bg-white px-4 text-sm font-semibold text-ink ring-1 ring-ink/10 hover:ring-clay/30">
                ...
                <input
                  ...
                  accept="image/png,image/jpeg,image/webp,image/gif"
                  onChange={(event) =>
                    uploadInto("gallery_image_urls", event.target.files?.[0])
                  }
                />
              </label>
            </div>
          ...
          <Field
            label="Tech stack"
            help="Optional. Write simple words separated by commas. Example: React, FastAPI, SQLite"
          >
            <Input
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Field
              ...
              help="Optional. You can paste domain.com; https:// is added."
            >
              <Input
            ...
            <Field
              ...
              help="Optional. You can paste github.com/user/repo."
            >
              <Input
...
function validateProject(form: typeof emptyProject) {
  ...
  if (form.slug.trim() && !slugify(form.slug))
    errors.slug = "Use letters, numbers, or dashes.";
  if (!isValidOptionalUrl(form.cover_image_url)) {
    errors.cover_image_url =
      "Use an upload URL, http(s) link, or leave it empty.";
  }
  const badGalleryUrl = splitList(form.gallery_image_urls).find(
    (url) => !isValidOptionalUrl(url),
  );
  if (badGalleryUrl) {
...
function buildProjectPayload(form: typeof emptyProject) {
  ...
  return {
    ...
    gallery_image_urls: splitList(form.gallery_image_urls).map(
...
function ContactsAdmin() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setStatus(
        err instanceof Error ? err.message : "Could not add contact link.",
      );
    }
  }
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
      <Card className="p-6">
        ...
        <p className="mt-2 text-sm leading-6 text-ink/60">
          Add simple contact rows like Email, LinkedIn, GitHub, Telegram, or
          Website. Type and URL can be inferred.
        </p>
        <form onSubmit={submit} className="mt-6 grid gap-3">
          <FormAlert
            message={status}
            tone={status?.includes("added") ? "success" : "error"}
          />
          <Field
            label="Label"
            error={errors.label}
            help="Example: Email, LinkedIn, GitHub, Website"
          >
            <Input
          ...
          <Field
            ...
            error={errors.value}
            help="What visitors will see. Example: linkedin.com/in/nurzhan"
          >
            <Input
          ...
          <Field
            label="Type"
            help={`Optional. Current guess: ${inferContactType(form.label, form.value)}.`}
          >
            <Input
          ...
          <Field
            label="Clickable URL"
            error={errors.url}
            help="Optional. If value is an email or website, this can be left empty."
          >
            <Input
...
function validateContactLink(form: {
  label: string;
  value: string;
  url: string;
}) {
  const errors: FieldErrors = {};
  if (!form.label.trim())
    errors.label = "Add a label, for example Email or LinkedIn.";
  if (!form.value.trim())
    errors.value = "Add the contact value visitors should see.";
  ...
  return (
    <Card className="p-6">
      ...
      <p className="mt-2 text-sm text-ink/60">
        Upload an image or PDF, then paste the returned URL into a project,
        profile, or resume field.
      </p>
      ...
      {result && (
        <div className="mt-5 rounded-md bg-ink/5 p-4">
          ...
          {mediaUrl(result) && (
            <a
              className="mt-3 block text-sm text-clay"
              href={mediaUrl(result) ?? "#"}
              target="_blank"
            >
              Preview file
            </a>
