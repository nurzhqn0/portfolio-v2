...
export function ProjectDetailPage() {
  ...
  if (isLoading) {
    return (
      <main className="mx-auto max-w-7xl px-5 py-24 text-center font-medium text-clay">
        Loading project...
      </main>
    );
  }
  ...
  if (!project) {
    return (
      <main className="mx-auto max-w-7xl px-5 py-24 text-center font-display text-4xl italic text-ink">
        Project not found.
      </main>
    );
  }
  ...
  return (
    <>
      <main className="relative mx-auto max-w-7xl overflow-hidden px-5 py-16">
        ...
        <Link
          to="/projects"
          className="group relative z-10 mb-12 inline-flex items-center gap-2 text-sm font-medium text-graphite transition-colors hover:text-clay"
        >
          <ArrowLeft
            size={18}
        ...
        <section className="relative z-10 grid gap-10 lg:grid-cols-[.9fr_1.1fr]">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {project.tech_stack.map((tech) => (
                <span
                  key={tech}
                  className="rounded-full bg-white px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-brand-navy shadow-sm ring-1 ring-brand-cyan/20"
                >
                  {tech}
                </span>
              ))}
          ...
          <div className="group relative aspect-[4/3] overflow-hidden rounded-[28px] border border-white bg-cover bg-center shadow-soft">
            <div
        ...
        {gallery.length > 0 && (
          <section className="relative z-10 mt-16 grid gap-6 md:grid-cols-2">
            {gallery.map((image) => (
              <div
                key={image}
                className="group overflow-hidden rounded-[24px] border border-white shadow-soft"
              >
                <img
                  src={image ?? ""}
                  alt=""
